...
def plt_output_unit(W, b):
    ...
    # Reshape the data to a 1D array and normalize it to range [0, 1]
    d_reshaped = d.flatten()
    d_normalized = (d_reshaped - np.min(d_reshaped)) / (np.max(d_reshaped) - np.min(d_reshaped))

    # Now, you can use the normalized data as the color values
    pcm = ax.scatter(x, y, z, c=d_normalized, cmap=cmap, alpha=1)
